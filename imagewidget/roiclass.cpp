...
ROIClass::ROIClass(TImage* snp, QWidget*p, QRect rec) :QWidget(p)//: TImageSnapshot(snp->makeSnpImageCat(rec, p))
{
   ...
   connect(sample->wgt, SIGNAL(newMarker(ImageMarker*)) ,this, SLOT(sendMarker(ImageMarker*)));
...
void ROIClass::sendMarker(ImageMarker* m) {
    QPoint p(m->_x, m->_y);
    if (!doubleMarker) {
        emit roiMarker(QPoint(p.x() + offset.x(), p.y() + offset.y()), m->_clr);
    }
    doubleMarker = false;
}
